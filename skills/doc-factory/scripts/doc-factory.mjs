  ...
  "node_modules/ignore/index.js"(exports, module) {
    ...
    var scanBracket = (pattern, start) => {
      ...
      for (; ; ) {
        ...
        if (char === ESCAPE) {
          const escaped = pattern[index2 + 1];
          ...
          prev = escaped;
          index2++;
        } else if (char === "-" && prev && index2 + 1 < length && pattern[index2 + 1] !== "]") {
          index2++;
          let to = pattern[index2];
          if (to === ESCAPE) {
            to = pattern[index2 += 1];
          }
          if (prev <= to) {
          ...
          prev = EMPTY;
        } else if (char === "[" && pattern[index2 + 1] === ":") {
          const nameStart = index2 + 2;
          let end = nameStart;
      ...
      while (index2 < length) {
        ...
        if (char === ESCAPE) {
          ...
          if (escaped === "*" || escaped === "[" || escaped === SPACE || escaped === ESCAPE) {
            out += pattern.slice(index2, index2 + 2);
          } else {
            out += hold(
          ...
          const scanned = scanBracket(pattern, index2);
          if (scanned === null) {
            out += hold(NEVER_MATCH);
            index2 = length;
          } else {
            out += hold(scanned.source);
            index2 = scanned.end + 1;
          }
        } else {
          out += char;
          index2++;
    ...
    var REPLACERS = [
      ...
      [
        ...
        (_, index2, str) => index2 + 6 < str.length ? str.slice(index2 + 6) === "\\/" ? "(?:\\/[^\\/]+)+" : "(?:\\/[^\\/]+)*" : "\\/.+",
        "*"
      ],
    ...
    var pinWildcards = (source) => {
      ...
      while (index2 < length) {
        ...
        if (source.startsWith(WILDCARD, index2)) {
          tokens.push({ wildcard: true });
          index2 += WILDCARD.length;
        } else if (char === "[") {
          let end = index2 + 1;
          if (source[end] === "^") {
            end++;
          ...
          tokens.push({ single: source.slice(index2, end) });
          index2 = end;
        } else if (char === ESCAPE) {
          tokens.push({ single: source.slice(index2, index2 + 2) });
          index2 += 2;
        } else if (char === "(") {
          let depth = 0;
          let end = index2;
          ...
          tokens.push({ boundary: source.slice(index2, end) });
          index2 = end;
        } else if (char === "^" || char === "$") {
    ...
    var makeRegexPrefix = (pattern) => {
      ...
      return sources.length ? replaced.replace(
        ...
        (match, index2) => sources[index2]
      ) : replaced;
    };
    var matchesBasename = (body) => {
      const index2 = body.indexOf(SLASH);
      return index2 < 0 || index2 === body.length - 1;
    };
    var basenameOf = (path5) => {
      const end = path5.length - 1;
      const index2 = path5.lastIndexOf(
        SLASH,
        path5[end] === SLASH ? end - 1 : end
      );
      return index2 < 0 ? path5 : path5.slice(index2 + 1);
    };
    var parentOf = (path5) => {
...
var require_tags = __commonJS({
  "node_modules/yaml/dist/schema/tags.js"(exports) {
    ...
    var schemas = /* @__PURE__ */ new Map([
      ["core", schema.schema],
      ["failsafe", [map3.map, seq.seq, string3.string]],
...
var excludedDirectories = /* @__PURE__ */ new Set([".git", "node_modules", "vendor", "dist", "build", "coverage", ".next", ".cache", ".venv", "__pycache__"]);
function hash(content3) {
  return crypto.createHash("sha256").update(content3).digest("hex");
}
...
function deniedPath(input) {
  const parts = input.split("/");
  return parts.some((part) => excludedDirectories.has(part.toLowerCase()) || /^\.doc-factory-/u.test(part) || /^\.env(?:\.|$)/iu.test(part) || /^(?:secrets?|credentials?)(?:\.|$)/iu.test(part) || /^(?:id_rsa|id_ed25519|\.npmrc|\.netrc)$/iu.test(part) || /\.(?:pem|key|p12|pfx|keystore)$/iu.test(part));
}
function sensitive(content3) {
  if (/-----BEGIN (?:[A-Z ]+ )?PRIVATE KEY-----|\bAKIA[0-9A-Z]{16}\b|\b(?:gh[pousr]_|github_pat_|sk-proj-)[A-Za-z0-9_\-]{16,}|\bBearer\s+[A-Za-z0-9._\-]{24,}/u.test(content3)) return true;
  const assignment = /(?:["']?)(?:password|passwd|api[_-]?key|access[_-]?token|client[_-]?secret|secret[_-]?key)(?:["']?)\s*[:=]\s*["']([^"'\n]{8,})["']/giu;
  return [...content3.matchAll(assignment)].some((match) => !/^(?:<[^>]+>|\$\{[^}]+\}|process\.env\..+|example.*|placeholder.*|your[-_ ].*|test[-_ ].*|changeme.*)$/iu.test(match[1]));
}
function assertSafeContent(content3) {
  if (Buffer.byteLength(content3) > MAX_FILE_BYTES) throw new FactoryError("too_large", "File exceeds the 2 MiB reading/writing limit.");
  if (content3.includes("\0") || content3.includes("\uFFFD")) throw new FactoryError("binary", "Only valid UTF-8 text is supported.");
  if (sensitive(content3)) throw new FactoryError("sensitive_content", "Content resembles a secret; it was withheld.");
}
function isDocument(file) {
  ...
  return /\.(?:md|mdx|rst)$/iu.test(file) && !/(?:^|\/)(?:SKILL|SYSTEM|CHANGELOG|LICENSE)(?:\.|$)/iu.test(file) || /(?:^|\/)(?:README|AGENTS)\.txt$/iu.test(file) || /^(?:docs?|documentation)\/.*\.txt$/iu.test(file);
}
function isNavigation(file) {
  return /(?:^|\/)(?:mkdocs\.ya?ml|_meta\.json|_category_\.json|SUMMARY\.md)$/u.test(file);
}
function generated(content3) {
  return /(?:auto[- ]generated|generated (?:file|by|from)|do not edit|@generated)/iu.test(content3.slice(0, 1500));
}
var Repository = class {
  root;
  gitRoot;
  constructor(root2) {
    this.root = fs.realpathSync(root2);
    if (!fs.statSync(this.root).isDirectory()) throw new FactoryError("not_directory", "Target must be a directory.");
    let top;
    try {
      top = this.git(["rev-parse", "--show-toplevel"]).trim();
    } catch {
      top = void 0;
    }
    this.gitRoot = top !== void 0 && fs.realpathSync(top) === this.root;
  }
  git(args, input) {
    try {
  ...
  resolve(file, allowMissing = false) {
    relativePath(file);
  ...
  ignored(file) {
    ...
    for (let depth = 0; depth < segments.length; depth++) {
      ...
      try {
        ...
        const matcher = (0, import_ignore.default)().add(rules);
  ...
  read(file) {
    ...
    try {
      ...
      const content3 = raw.toString("utf8");
      assertSafeContent(content3);
      return { path: file, hash: hash(raw), content: content3 };
    } finally {
      fs.closeSync(fd);
    }
  }
  readRevision(file, ref) {
    ...
    const revision2 = this.git(["rev-parse", "--verify", "--end-of-options", `${ref}^{commit}`]).trim();
    const listing = this.git(["ls-tree", "-z", revision2, "--", file]);
    if (!/^100(?:644|755) blob /u.test(listing)) throw new FactoryError("historical_file", "Historical target must be a regular file.");
    const content3 = this.git(["show", `${revision2}:${file}`]);
    assertSafeContent(content3);
    return { path: file, hash: hash(content3), content: content3, revision: revision2 };
  }
  inventory() {
    const files = [];
    const skipped = [];
    const visit2 = (dir) => {
      for (const entry of fs.readdirSync(path.join(this.root, dir), { withFileTypes: true })) {
        ...
        try {
          ...
          if (entry.isDirectory()) visit2(file);
...
function entries(repo, args, before, after) {
  ...
  for (let i = 0; i < tokens.length && tokens[i]; ) {
    ...
    const result = { status, path: file, ...moved ? { oldPath: first } : {} };
    try {
...
function delta(repo, scope = {}) {
  ...
  const empty2 = repo.git(["hash-object", "-t", "tree", "--stdin"], "").trim();
  const baseHead = head ?? empty2;
  let from = baseHead;
  let to = "WORKTREE";
  let mode = "worktree";
  if (scope.base) {
    from = revision(repo, scope.base);
    mode = "base";
  }
  if (scope.range) {
  ...
  const committed = mode === "range" ? entries(repo, [from, to], from, to) : mode === "base" ? entries(repo, [from, baseHead], from, baseHead) : [];
  ...
  if (mode !== "range") {
    for (const file of repo.git(["ls-files", "--others", "--exclude-standard", "-z"]).split("\0").filter(Boolean)) {
      ...
      try {
        const text5 = repo.read(file);
        untracked.push({ status: "?", path: file, patch: text5.content });
      } catch (error) {
        if (!(error instanceof FactoryError)) throw error;
...
function markdown(file, content3) {
  const issues = [];
  let body = content3;
  const frontmatter = /^(?:\ufeff)?---\r?\n([\s\S]*?)\r?\n---(?:\r?\n|$)/u.exec(content3);
  if (frontmatter) {
    const yaml = (0, import_yaml.parseDocument)(frontmatter[1]);
    if (yaml.errors.length) issues.push({ severity: "error", code: "frontmatter", path: file, message: "Invalid YAML frontmatter." });
    body = content3.slice(frontmatter[0].length);
  }
  const root2 = unified().use(remarkParse).use(remarkGfm).parse(body);
  const links = [];
  const anchors = /* @__PURE__ */ new Set();
  ...
  const text5 = (node2) => node2.value ?? (node2.children ?? []).map(text5).join("");
  const walk = (node2) => {
    if (node2.type === "heading") anchors.add(slugger.slug(text5(node2)));
    if ((node2.type === "paragraph" || node2.type === "table") && text5(node2).trim().length >= 20) meaningful = true;
    if (node2.type === "code" && (node2.value ?? "").trim().length > 10) meaningful = true;
    if ((node2.type === "link" || node2.type === "image") && node2.url) links.push(node2.url);
    if (node2.type === "definition" && node2.identifier && node2.url) definitions.set(node2.identifier.toLowerCase(), node2.url);
    if ((node2.type === "linkReference" || node2.type === "imageReference") && node2.identifier) references.push(node2.identifier.toLowerCase());
    if (node2.type === "html") {
      for (const match of (node2.value ?? "").matchAll(/\b(?:id|name)=["']([^"']+)["']/gu)) anchors.add(match[1]);
      if (/\b(?:href|src)=/u.test(node2.value ?? "")) issues.push({ severity: "unverified", code: "html_link", path: file, message: "HTML attribute links require renderer-specific verification." });
    }
...
function validateDocuments(repo, overlays = /* @__PURE__ */ new Map(), entrypoints = []) {
  ...
  for (const [file, page] of pages) {
    ...
    for (const raw of page.links) {
      ...
      try {
        ...
        fragment = rawFragment === void 0 ? void 0 : decodeURIComponent(rawFragment);
      } catch {
        issues.push({ severity: "error", code: "link_encoding", path: file, target: raw, message: "Invalid URL encoding." });
        continue;
      }
      let target = location ? path3.posix.normalize(path3.posix.join(path3.posix.dirname(file), location)) : file;
      try {
        const absolute = repo.resolve(target, true);
        if (!overlays.has(target)) {
          if (!fs2.existsSync(absolute)) throw new FactoryError("missing_link", "Missing link target.");
          if (fs2.statSync(absolute).isDirectory()) {
...
function parseProposal(value) {
  if (!object(value) || value.version !== 1 || !["init", "update"].includes(String(value.workflow)) || !Array.isArray(value.changes) || !Array.isArray(value.entrypoints) || !value.entrypoints.length || !value.entrypoints.every((item) => typeof item === "string")) {
    throw new FactoryError("invalid_proposal", "Expected version 1, workflow, changes, and nonempty entrypoints.");
  }
  if (value.scope !== void 0 && (!object(value.scope) || Object.keys(value.scope).some((key) => !["base", "range"].includes(key)) || Object.values(value.scope).some((item) => typeof item !== "string" || !item))) throw new FactoryError("invalid_proposal", "Invalid scope.");
  ...
  if (value.gaps !== void 0 && (!Array.isArray(value.gaps) || !value.gaps.every((item) => typeof item === "string"))) throw new FactoryError("invalid_proposal", "Gaps must be strings.");
  const seen = /* @__PURE__ */ new Set();
  for (const item of value.changes) {
    if (!object(item) || typeof item.path !== "string" || typeof item.content !== "string" || typeof item.reason !== "string" || !item.reason.trim() || !(item.expectedHash === null || validHash(item.expectedHash)) || !Array.isArray(item.evidence) || !item.evidence.length || item.evidence.some((ref) => !object(ref) || typeof ref.path !== "string" || !validHash(ref.hash) || ref.revision !== void 0 && typeof ref.revision !== "string") || item.kind !== void 0 && !["document", "navigation"].includes(String(item.kind)) || Object.keys(item).some((key) => !["path", "expectedHash", "content", "reason", "evidence", "kind"].includes(key))) {
  ...
  for (const file of value.entrypoints) relativePath(file);
  for (const gap of value.gaps ?? []) assertSafeContent(gap);
  return value;
}
function staticData(file, content3) {
  try {
    const parsed = file.endsWith(".json") ? JSON.parse(content3) : (() => {
      const result = (0, import_yaml2.parseDocument)(content3);
    ...
    throw new FactoryError("navigation_format", "Navigation must be a static JSON/YAML mapping without custom tags.");
  }
}
function verifyNavigation(file, oldContent, content3) {
  ...
  const previous3 = staticData(file, oldContent);
  const next = staticData(file, content3);
  if (/mkdocs\.ya?ml$/u.test(file)) {
    delete previous3.nav;
    delete next.nav;
    if (JSON.stringify(previous3) !== JSON.stringify(next)) throw new FactoryError("navigation_scope", "Only the mkdocs nav field may change.");
  }
}
function verifyChange(repo, change, ownWrites = /* @__PURE__ */ new Map()) {
  ...
  const exists = fs3.existsSync(absolute);
  const previous3 = exists ? repo.read(change.path) : null;
  if ((previous3?.hash ?? null) !== change.expectedHash) throw new FactoryError("conflict", `Document changed since it was read: ${change.path}`);
  if (previous3 && generated(previous3.content)) throw new FactoryError("generated_document", "Generated documents must be maintained at their source.");
  if (change.kind === "navigation") {
    if (!previous3) throw new FactoryError("navigation_scope", "Only existing recognized navigation configuration may be edited.");
    verifyNavigation(change.path, previous3.content, change.content);
  } else {
    ...
    if (!previous3 && /^(?:TODO|TBD|待填写|占位)(?:\s|[.:：]|$)/imu.test(change.content)) throw new FactoryError("placeholder", "New documentation must not contain placeholder sections.");
  }
  ...
  return previous3?.content ?? null;
}
function checkScope(repo, proposal, dryRun) {
  ...
  if (!dryRun && current.mode === "range") {
    for (const change of proposal.changes) {
      for (const evidence of change.evidence.filter((ref) => !isDocument(ref.path))) {
        let content3;
        try {
          content3 = repo.git(["show", `${current.to}:${relativePath(evidence.path)}`]);
        } catch {
          throw new FactoryError("historical_evidence", "Range evidence must exist at the selected end revision.");
        }
        if (hash(content3) !== evidence.hash) throw new FactoryError("historical_evidence", "Range evidence differs from the selected end revision.");
      }
    }
  }
}
function issueKey(issue) {
  return JSON.stringify([issue.code, issue.path, issue.target]);
}
function checkProposal(repo, proposal, dryRun = true) {
...
function applyProposal(repo, proposal, dryRun = false) {
  const checked = checkProposal(repo, proposal, dryRun);
  if (!checked.ok) return { ...checked, status: "invalid", written: [] };
  if (dryRun) return { ...checked, status: "dry_run", written: [] };
  const written = [];
  if (!checked.patches.length) return { ...checked, status: "no_change", written };
  const lockPath = path4.join(repo.root, ".doc-factory-write.lock");
  let lock;
  try {
    lock = fs3.openSync(lockPath, "wx", 384);
  } catch {
    throw new FactoryError("locked", "Another write or an interrupted run owns .doc-factory-write.lock; inspect it before retrying.");
  }
  const temporaryPaths = /* @__PURE__ */ new Set();
  const ownWrites = /* @__PURE__ */ new Map();
  const cleanupErrors = [];
  let outcome;
  try {
    fs3.writeFileSync(lock, JSON.stringify({ pid: process.pid }));
    ...
    for (const change of proposal.changes) {
      ...
      const previous3 = verifyChange(repo, change, ownWrites);
      const absolute = repo.resolve(change.path, true);
      fs3.mkdirSync(path4.dirname(absolute), { recursive: true });
      repo.resolve(change.path, true);
      const temp = path4.join(path4.dirname(absolute), `.doc-factory-${crypto2.randomUUID()}.tmp`);
      const mode = previous3 === null ? 420 : fs3.statSync(absolute).mode & 511;
      const fd = fs3.openSync(temp, "wx", mode);
      temporaryPaths.add(temp);
      try {
        fs3.writeFileSync(fd, change.content);
        fs3.fsyncSync(fd);
      } finally {
        fs3.closeSync(fd);
      }
      verifyChange(repo, change, ownWrites);
      if (previous3 === null) fs3.linkSync(temp, absolute);
      else fs3.renameSync(temp, absolute);
      written.push(change.path);
      ownWrites.set(change.path, hash(change.content));
      if (previous3 === null) fs3.unlinkSync(temp);
    ...
    const after = validateDocuments(repo, /* @__PURE__ */ new Map(), proposal.entrypoints);
    ...
    outcome = {
      ...checked,
      ok: false,
      status: written.length ? "partial" : "conflict",
      written,
    ...
    for (const temporary of temporaryPaths) {
      try {
        ...
        if (error.code !== "ENOENT") cleanupErrors.push(path4.relative(repo.root, temporary));
      }
    }
    try {
      fs3.closeSync(lock);
    } catch {
      cleanupErrors.push("writer lock handle");
    }
    try {
      fs3.unlinkSync(lockPath);
    } catch {
      cleanupErrors.push(".doc-factory-write.lock");
    }
  }
  ...
  if (values.help || !command) console.log(JSON.stringify(help, null, 2));
    ...
    switch (command) {
      case "scan": {
        ...
        if (!Number.isSafeInteger(offset) || offset < 0 || !Number.isSafeInteger(limit) || limit < 1 || limit > 2e3) throw new FactoryError("pagination", "Offset must be nonnegative; limit must be between 1 and 2000.");
        const inventory = repo.inventory();
        ...
        output = {
          ...
          files: files.slice(offset, offset + limit).map((file) => ({ path: file, kind: isNavigation(file) ? "navigation" : isDocument(file) ? "document" : "evidence" })),
      ...
      case "diff": {
        ...
        const limit = Number(values.limit);
        if (!Number.isSafeInteger(offset) || offset < 0 || !Number.isSafeInteger(limit) || limit < 1 || limit > 2e3) throw new FactoryError("pagination", "Offset must be nonnegative; limit must be between 1 and 2000.");
        const layers = ["committed", "staged", "unstaged", "untracked"];
        ...
        for (const layer of layers) result[layer] = selected.filter((item) => item.layer === layer).map(({ entry }) => {
          ...
          return { ...rest, ...patch === void 0 ? {} : { patchOmitted: true } };
        });
        output = { ...result, pagination: { total: stream.length, offset, limit, nextOffset: offset + limit < stream.length ? offset + limit : null } };
        break;
      }
      case "check":
        output = values.proposal ? checkProposal(repo, proposal()) : { issues: validateDocuments(repo), semanticCorrectness: "not_proven" };
        break;
      case "apply":
        output = applyProposal(repo, proposal(), values["dry-run"]);
        break;
      default:
        throw new FactoryError("unknown_command", "Unknown helper command. init/update are Skill workflows; use --help.");
    }
    const serialized = JSON.stringify(output, null, 2);
    console.log(serialized);
    if (typeof output === "object" && output !== null && "ok" in output && output.ok === false) process.exitCode = 1;
    if (command === "check" && !values.proposal && output.issues.some((issue) => issue.severity === "error")) process.exitCode = 1;
  }
} catch (error) {
